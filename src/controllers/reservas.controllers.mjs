...
const createReservas = async ( req, res ) => {
    ...
    res.status( 201 ) .json( data );
      ...
      return res.status(400).json({ msg: 'Ya existe una reserva con este usuario.' });
    }
        console.error( error ) ;
...
const updateReservaById = async (req, res) => {
    ...
    try {
        ...
        if (! reservaFound) {
            return res.status(404).json({ msg: 'La reserva que deseas actualizar no existe' });
        }

        // Paso 2: Actualizar el usuario si existe
        const data = await reservasModel.findByIdAndUpdate(reservaId, inputData, { new: true });
        res.status(200).json(data);
...
const deleteReservaById = async (req, res) => {
    ...
    try {
        ...
        if (!reservaFound) {
            return res.status(404).json({ msg: 'La reserva que deseas eliminar no existe' });
        }
